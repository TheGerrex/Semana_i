/**
 Stencil Runtime v0.0.0-dev.20191023015347
...
const attachStyles = (elm, cmpMeta, mode) => {
    ...
    if ((appData.BUILD.shadowDom || appData.BUILD.scoped) && appData.BUILD.cssAnnotations && cmpMeta.$flags$ & 10 /* needsScopedEncapsulation */) {
        ...
        if (appData.BUILD.scoped && cmpMeta.$flags$ & 2 /* scopedCssEncapsulation */) {
    ...
    const walk = (c) => {
        for (let i = 0; i < c.length; i++) {
            ...
            if (Array.isArray(child)) {
                ...
                else if (appData.BUILD.isDev && child.$flags$ === undefined) {
...
const setAccessor = (elm, memberName, oldValue, newValue, isSvg, flags) => {
    ...
    else if (appData.BUILD.vdomListener && !isProp && memberName[0] === 'o' && memberName[1] === 'n') {
        // Event Handlers
    ...
    if (appData.BUILD.hasRenderFn || appData.BUILD.reflect) {
        if (appData.BUILD.vdomRender || appData.BUILD.reflect) {
            try {
                ...
                // DOM WRITE!
                renderVdom(elm, hostRef, cmpMeta, (appData.BUILD.allRenderFn) ? instance.render() : (instance.render && instance.render()));
            }
            catch (e) {
            ...
            elm.textContent = (appData.BUILD.allRenderFn) ? instance.render() : (instance.render && instance.render());
        }
    }
    if (appData.BUILD.cssVarShim && platform.plt.$cssShim$) {
...
const setValue = (ref, propName, newVal, cmpMeta) => {
    ...
    if (newVal !== oldVal && (!appData.BUILD.lazyLoad || !(flags & 8 /* isConstructingInstance */) || oldVal === undefined)) {
    ...
    if ((platform.plt.$flags$ & 1 /* isTmpDisconnected */) === 0) {
        ...
        if (!(hostRef.$flags$ & 1 /* hasConnected */)) {
            ...
            if (appData.BUILD.slotRelocation && !hostId) {
                ...
                if ((appData.BUILD.hydrateServerSide) ||
                    (appData.BUILD.slot && cmpMeta.$flags$ & 4 /* hasSlotRelocation */) ||
                    (appData.BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */)) {
...
const createEvent = (ref, name, flags) => {
    ...
    return {
        emit: (detail) => {
            ...
            return elm.dispatchEvent(new (appData.BUILD.hydrateServerSide ? platform.win.CustomEvent : CustomEvent)(name, {
